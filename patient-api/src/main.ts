...
// Stripe Connect endpoints moved to endpoints/stripe-connect.ts
// Stripe webhook endpoint moved to endpoints/stripe-webhook.ts
// Users endpoints moved to endpoints/users.ts
// Orders endpoints moved to endpoints/orders.ts
// Brand Subscriptions endpoints moved to endpoints/brand-subscriptions.ts
// Tenant Products endpoints moved to endpoints/tenant-products.ts
...
// Patient endpoints moved to endpoints/patient.ts
// Order endpoints moved to endpoints/orders.ts
...
// Users profile endpoint moved to endpoints/users.ts
...
async function startServer() {
  ...
  const { registerStripeConnectEndpoints } = await import('./endpoints/stripe-connect');
  registerStripeConnectEndpoints(app, authenticateJWT, getCurrentUser);

  // ============= STRIPE WEBHOOK ENDPOINT =============
  const { registerStripeWebhookEndpoint } = await import('./endpoints/stripe-webhook');
  registerStripeWebhookEndpoint(app, stripe);

  // ============= USERS ENDPOINTS =============
  const { registerUsersEndpoints } = await import('./endpoints/users');
  registerUsersEndpoints(app, authenticateJWT, getCurrentUser);

  // ============= ORDERS ENDPOINTS =============
  const { registerOrdersEndpoints } = await import('./endpoints/orders');
  registerOrdersEndpoints(app, authenticateJWT, getCurrentUser);

  // ============= TENANT PRODUCTS ENDPOINTS =============
  const { registerTenantProductsEndpoints } = await import('./endpoints/tenant-products');
  registerTenantProductsEndpoints(app, authenticateJWT, getCurrentUser);

  // ============= BRAND SUBSCRIPTIONS ENDPOINTS =============
  const { registerBrandSubscriptionsEndpoints } = await import('./endpoints/brand-subscriptions');
  registerBrandSubscriptionsEndpoints(app, authenticateJWT, getCurrentUser, stripe);

  // ============= PATIENT ENDPOINTS =============
  const { registerPatientEndpoints } = await import('./endpoints/patient');
  registerPatientEndpoints(app, authenticateJWT, getCurrentUser);
